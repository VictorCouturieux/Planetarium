    ...
    public class Carpet : MonoBehaviour
    {
        ...
        void Update(){
            if(activeBlobs.Count > 0){
                targetVolume = sliderPos;
            }
            else { targetVolume = currentSoundVolume;}

            if ( Math.Abs(targetVolume - currentSoundVolume)<0.01){
                currentSoundVolume = targetVolume;
            }
            else{
                currentSoundVolume+=(targetVolume-currentSoundVolume)/400.0f;
            }
        ...
        private void OnSerialValues(string[] values) {
            ...
            if (ParseBlob(values, out blob)) {
                if (blob.state && (blob.box.z > blobMinimumDepth) && (blob.centroid.x>minx && blob.centroid.x<maxx && blob.centroid.y>miny && blob.centroid.y<maxy)) {
            ...
            if ((activeBlobs.Count > 0) && GetAverageBlob(out averageBlob)) {
                posy = averageBlob.centroid.y ;
                posx = averageBlob.centroid.x ;

                float fingerAbsolutePos = (averageBlob.centroid.y-miny)/(maxy-miny);
                sliderPos = fingerAbsolutePos;
            }
        }
